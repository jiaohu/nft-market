...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, HasTokenURI {
    ...
    mapping(uint256 => bool) internal _isListing;
    ...
    function _listingNFT(uint256 tokenId) internal virtual {
        ...
        require(_owners[tokenId] == msg.sender, "ListingNFT error: INVALID_OPERATOR, caller not have the asset");
        require(_isListing[tokenId] == false, "ListingNFT error: INVALID_OPERATOR, the asset has listed");
        // 将其设置为listing
        _isListing[tokenId] = true;
    }

    function _cancelListNFT(uint256 tokenId) internal virtual {
        require(_owners[tokenId] == msg.sender, "ListingNFT error: INVALID_OPERATOR, caller not have the asset");
        require(_isListing[tokenId] == true, "ListingNFT error: INVALID_OPERATOR, the asset not listed");
        _isListing[tokenId] = false;
    }

    function _listingStatus(uint256 tokenId) internal view returns (bool) {
        return _isListing[tokenId];
    }
