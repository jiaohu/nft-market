 ...
 */
abstract contract ERC1155Meta is ERC1155, SignatureValidator {
    ...
    public
  {
    ...
    if (_isGasFee) {
      ...
      _doSafeTransferAcceptanceCheck(msg.sender, _from, _to, _id, _amount, transferData);

      // Transfer gas cost
      _transferGasFee(_from, gasReceipt);

    } else {
      _doSafeTransferAcceptanceCheck(msg.sender, _from, _to, _id, _amount, signedData);
    }
  }
    ...
    public
  {
    ...
    if (_isGasFee) {
      ...
      // limit. Nothing can prevent the receiver to revert the transaction as close to the gasLimit as
      // possible, but the relayer can now only accept meta-transaction gasLimit within a certain range.
      _doSafeBatchTransferAcceptanceCheck(msg.sender, _from, _to, _ids, _amounts, transferData);

      // Handle gas reimbursement
      _transferGasFee(_from, gasReceipt);

    } else {
      _doSafeBatchTransferAcceptanceCheck(msg.sender, _from, _to, _ids, _amounts, signedData);
    }
  }
      ...
      internal
  {
    ...
    if (feeTokenType == FeeTokenType.ERC1155) {
      ...
      if (tokenAddress == address(this)) {
        ...
        _doSafeTransferAcceptanceCheck(msg.sender, _from, feeRecipient, tokenID, fee, "");
